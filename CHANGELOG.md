...
* feat: `data-loop-presentation` on `<main>` — at the last frame, loop back to the first instead of stopping (kiosk / exhibition playback). Toggle live with <kbd>Shift+L</kbd>; presettable from the hash (`#&state=loop-presentation`). Distinct from the narrower-scoped, existing per-frame `data-loop` (image animation)
