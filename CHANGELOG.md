...
### Embedding
* feat: experimental `?embed` mode for hosting slidershow in an iframe on a foreign page – a `postMessage` bridge (`_load`/`_open`, settled `close`/`destroy`) replaces the presentation and shows/hides the overlay without the splash screen; `close()` in picker mode (`_open({mode:"picker"})`) reports back every frame tagged `1` as `{selected:[{id,tags}]}`
* feat: `_open()` grabs keyboard focus into the iframe (a `postMessage` command carries no user gesture, so hotkeys/arrows previously stayed wherever focus was on the host page); `_open({grid:true})` starts straight in the grid overview; <kbd>Escape</kbd> now reports `close()` instead of toggling the in-app menu, which the embed contract keeps hidden (including from the grid overview's own <kbd>Escape</kbd> binding); the control-icons row (prev/next/grid, top-right) grows its own ✕ close icon in embedded mode instead of the host having to draw one
...
* feat: **data saver** (<kbd>Alt+B</kbd>, the 🐢 menu button, or `save-data` in the URL hash) – for a metered or slow connection. Full-size photos are not downloaded: where `sli-thumb` thumbnails exist the thumbnail becomes the picture (an album costs megabytes instead of gigabytes), and where none exists nothing is loaded until <kbd>Alt+L</kbd> asks for that one frame. Switches itself on when the browser reports a metered connection (`navigator.connection.saveData`); a manual toggle wins from then on. A 🐢 badge shows the mode and whether the current frame is standing in for something; clicking it (<kbd>Shift+Alt+B</kbd>) reports how much was saved – the bytes really transferred measured exactly, the ones skipped honestly marked as an estimate. In the mode the read-ahead window also shrinks from 50/20 frames to 5/2, a 2G-class connection counts as metered even without the Save-Data flag, and turning it on in a presentation with no `sli-thumb` at all says so upfront
* fix: a media fetch that finished after its frame had been unloaded could still paint itself over whatever the element held by then – every load now carries a token that a later unload invalidates
* fix: opening the grid overview (<kbd>G</kbd>) left a playing video playing – and sounding – behind it, since opening the grid does not leave the frame. It is paused now, and closing the grid back onto the same frame resumes it (a video already paused stays paused, and one left behind by navigating elsewhere in the grid is not restarted)

### Export
* feat: <kbd>Ctrl+S</kbd> now has a **single Export button** instead of five. What used to be five buttons – each silently governed by a different subset of the radios around it – is a plain "Media:" choice (referenced where they are / one single file / a media folder / folders by tags), and the settings a choice does not reach are visibly greyed out instead of leaving you to remember which button they applied to
* feat: a fourth "App's own code" option – **"Point at a copy I already have"**: give a relative directory (ex: `../lib/slidershow/`) and the exported file loads SlideRshow from there instead of the CDN. Nothing is copied or fetched, so unlike "Inline"/"Copy into a folder" it also works from a `file://` presentation, and several presentations can share one offline copy
* feat: a fifth "App's own code" option – **"Leave as-is"**: keeps the exported bootstrap `<script src>` exactly as this session already has it, instead of force-rewriting it to the CDN. It's now also the default whenever the export looks like it's coming from your own dev machine (`presenter.html` opened via `file://`, or a `localhost` dev server) – exporting from there no longer silently discards a local dev pin. A real self-hosted embed (ex. a WordPress plugin on a live domain) still defaults to the CDN rewrite, so an export opened elsewhere doesn't 404 on a plugin-only relative path
* feat: "Another folder" is no longer a second prompt dialog – the media folder path is a field right in the export dialog, prefilled from `sli-path`. Leaving it empty is what "Same folder" used to be, so the two collapse into one
* fix: exporting one and the same presentation cost a second one time and gigabytes of RAM (up to an "Aw snap", leaving the overwritten file at 0 bytes) the next, depending on how much of the grid overview had been scrolled through meanwhile. The export copy was made by re-parsing `body.outerHTML` in the live document, which instantiates every `<img>`/`<video>` in it – including the hundreds of grid thumbnails the HUD piles up – so the browser fetched and decoded them all just to have them dropped again a moment later. The copy is now cloned into an inert document, which loads nothing and skips building that huge intermediate HTML string too
* fix: the exported file is assembled as a `Blob` of its parts and streamed out in chunks instead of being concatenated into one giant string and written in a single piece – that used to make two extra full copies of the whole presentation, and a big single-file export could even hit the hard ~512M-character ceiling of a JS string
* fix: the export progress bar ran off its scale on any presentation holding a frame with more than one photo/video in it
* fix: after exporting a presentation of dragged-in files, the grid previews (and any media not already painted) went blank in the still-open tab – "unloading the export copy" revoked the `blob:` URLs, which the copy shares with the live elements it was serialized from. Only the copy is stripped now

### Properties
* feat: <kbd>Alt+V</kbd> asks what the video point should do instead of only snapshotting what the video happens to be doing – it pauses the video, offers `goto` / `rate` / `pause` / `mute`+`unmute` / zoom, and resumes the playback afterwards, so a whole rule set can be marked without ever leaving the video. Rate and sound are pre-filled only where the video differs from what the earlier points already put in effect. Marking a second time offers to fill in the **previous** point's missing `goto` instead of adding a point of its own (that is what cutting a video into excerpts needs, and it is the default when that point carries nothing but its time). <kbd>Shift+Alt+V</kbd> keeps the old dialog-less snapshot; clicking a video point in the properties panel opens the same dialog (with Remove, and "Zoom live" to hand over to the panel's live editing)
* fix: clicking a video point carrying a `goto` in the properties panel jumped the video to the goto target and then silently rewrote the point's own `startTime` to it
* feat: a frame's step-points / video-points are mirrored as read-only pills in the top-right corner while the properties panel is closed (editing mode only), so it is visible at a glance that a frame carries points – and that <kbd>Alt+S</kbd>/<kbd>Alt+V</kbd> just added one. Clicking the badge opens the panel to edit them
* fix: <kbd>Alt+S</kbd> / <kbd>Alt+V</kbd> silently did nothing when the properties panel had never been opened for the frame – the point editor they drive is built by the panel. The rows are now built (still hidden) on demand, and a stale editor left over from a previous frame is dropped instead of being edited by mistake
...
* fix: the per-property documentation was dead ever since the reference moved out of `README.md` into the MkDocs site – the panel fetched the README and found nothing. It now reads the docs/*.md pages and the “→ docs” link points to the property's real published anchor
* enh: that documentation now unfolds inline under the property's own row (a ⓘ next to the label makes the click discoverable) instead of flashing by as a HUD notification over the very field you were reading about
* feat: the properties panel (<kbd>Alt+P</kbd>) is grouped instead of one long flat list – Frame/Section/Main tabs (only shown when they'd actually hold something), each with collapsible Notes/Transform/Video/Timing sections, plus a filter box that jumps straight to a property by name across every tab and group
* feat: <kbd>Alt+V</kbd> adds a video-point at the video's current playback time, the video counterpart to the existing <kbd>Alt+S</kbd> step-point shortcut – both now open the panel first if it was closed, instead of silently doing nothing (or, previously, guessing at a stale button left over from a different frame)
* feat: <kbd>Alt+,</kbd> / <kbd>Alt+.</kbd> mark a video's trim start/end (`sli-video-cut`) at the current playback time without opening the panel at all; both are undoable (changed from <kbd>Alt+[</kbd> / <kbd>Alt+]</kbd> for Czech keyboard compatibility)
* fix: <kbd>Alt+S</kbd> / <kbd>Alt+V</kbd> no longer auto-open the properties panel – the points are created silently, letting you decide when to view them
* feat: step-points and video-points can be dragged to reorder them, instead of only via a full delete-and-re-add
* fix: <kbd>Alt+S</kbd> used to click "whichever + button happens to be second in the DOM" – fragile once a property duplicates across ancestor levels, and wrong outright after switching frames with the panel closed. It now targets the current frame's own point editor directly
* fix: the frame's own `rotate`/`step-points`/`video-points`/`playback-rate`/`video` row only showed a bare ancestor tag ("- ARTICLE:") with no property name, making stacked rows indistinguishable; inactive Section/Main tab buttons were unreadable (black text on near-black)
* fix: `video-cut` always read back empty for a video with `sli-src` set (the common case for imported media) – it was read via `get_filename()`, which prefers `sli-src`, but the trim fragment only ever lives on `src`
* enh: that same duplicate row is now hidden by default when there's only one image/video in the frame and no value set there yet (the case it actually matters – more than one media element per frame – is rare), with a "+ set for the whole frame too" link to reveal it
...
* fix: dropping a big folder in used to freeze the browser and eventually kill the tab – every dropped file was copied into the memory in full (a 45 GB folder meant 45 GB of RAM) and *all* of the frames were preloaded at once, however far away. Files are now referenced where they lie on the disk and only the usual preload window is loaded up front, so thousands of photos and videos are imported without the memory going anywhere. EXIF reading is throttled as well, and building thousands of frames still freezes the tab for a while – a full-screen "Importing…" banner now covers that stretch instead of a corner spinner easy to miss, and the progress circle shows a percentage instead of a raw count so it no longer spills outside the ring on a five-figure import
* fix: a photo whose EXIF data reaches past the header slice we read for it (large embedded thumbnail, verbose MakerNote) made the vendored EXIF reader throw an uncaught error – harmless (that file just falls back to its own file date) but alarming during a big import, since it read like a crash and popped a toast per affected file. Downgraded to one quiet console line

### Performance
* fix: any re-layout of a large presentation (sorting, grouping, renaming, deleting or importing frames – anything that reruns `reset()`) froze the browser for tens of seconds on ten thousand frames, and the more thumbnails the grid held, the longer it took. Every frame looked its preview up by scanning the whole document; previews are kept in a lookup table now, so the pass is linear again (11 000 frames with a fully loaded grid: 4.4 s → 0.1 s)
* fix: deleting frames one by one from the grid (the tile's ✖ button) re-ran the whole `reset()` per click; a burst of consecutive deletes now coalesces into a single reset. `reset()` itself is also leaner – frame repositioning writes styles directly instead of through jQuery, and tags are looked up from one localStorage scan instead of one read per frame (8 000 frames: reset() 300 ms → 130 ms)

### Grid
* feat: sticky section headers – when scrolling through the grid, the current section's header stays pinned to the top, so you always know which section you're in. Ribbons are now built for the whole presentation up front (they carry no media, so this is cheap) instead of streaming in with the frame thumbnails – otherwise the current section's own header could sit outside the lazy-loaded window and never appear at all when the grid opened scrolled deep into it, and the container's true scrollable height would run far past the loaded thumbnails, stalling further loading once scrolled past them
* feat: a section's ▶/▼ toggle (or <kbd>Alt+O</kbd> for the current one) collapses it in place, hiding its frames and any nested subsections behind a single-line ribbon – and "collapse all"/"expand all" on the presentation's or a section's own menu does the same for everything nested under it at once. A collapsed ribbon still stays sticky, so a long album can be scanned section by section without loading a single thumbnail
* fix: a sticky section ribbon spilled past both edges of the grid instead of spanning it – its horizontal margin/padding added on top of an already-100%-wide flex box
* fix: every ribbon reserved space for two rows of hover-only menu buttons whether or not they were showing, making each sticky header far taller than the single line of text it actually needed
* fix: a ribbon's hover-menu dropdown (sort/regroup/…) rendered clipped and unreachable – the single-line collapse trick above hid it with `overflow:hidden`, cutting off the dropdown along with the row it was collapsing
* fix: the "Presentation" ribbon no longer stays stuck to the top alongside the current section's – it isn't a section itself and stacking it as a second sticky row said nothing a section header didn't already
* fix: a ribbon's hover-menu dropdown was cut through by a black band – the sticky ribbon below it painted over it; and a dropdown pushed down to clear the second menu row closed before the pointer could reach it, the gap in between not counting as hover any more
* enh: while the grid is open, thumbnails load nearest-to-the-scroll-position first (until now: nearest to the frame being played), and the grid stops paging further batches in while the tiles already queued are still fetching – scrolling used to leave the section on screen blank while thumbnails for sections far below it were being fetched, the growing grid carrying the presenter far past what they were looking at
* enh: the ribbon menu whose dropdown is showing has its own label marked, so it is clear whether the open list belongs to "sort" or to "order" just below it
* fix: scrolling up through a long presentation could visibly jump backward and seem to loop over the same thumbnails – loading earlier frames re-anchors the scroll position to stay put, but the browser's own CSS scroll anchoring was doing the exact same correction at the same time, so both applied and overshot past the intended spot
* fix: expanding a section far from wherever the grid's lazy-loaded window last was showed nothing – merely un-hiding its frames does nothing when none were ever rendered there to begin with; expanding now also loads that section's own frames and scrolls to them
* feat: "regroup by …" orders sections in either direction now (⇑/⇓ per criterion) – tag/folder/camera/date sections used to only come out in whichever order the regroup happened to build them in
* fix: regrouping by hours/days/weeks/months/years lumped every photo without EXIF into a single "unknown" section – the file's own modification date, which such a photo falls back to, is written in a form the date parser rejected
* fix: the mouse wheel over a video tile scrolls the grid again. The film-camera badge marking a video is sized for the shrunken clone of a whole screen, but a video-only frame's tile is not scaled that way, so the badge landed there unshrunk at 300px – spilling out of the cell and turning it into a scroll box of its own, which swallowed the wheel. It now sizes itself against the tile, and a preview never scrolls at all
* fix: sorting subsections alphabetically re-appended one section at a time, reflowing the whole document per section – a sort spanning thousands of subsections now moves each parent's children in a single batch
* fix: "regroup by tags/date" re-ran a document-wide selector to find each frame's target section – same O(n²) class of freeze as the thumbnail lookup above, now a lookup table built once per regroup
* fix: a long-press on a thumbnail now selects it, the touch equivalent of Ctrl/Shift-click – there was previously no way to build a multi-selection on mobile at all, since both the modified click and the drag-marquee only listen for a mouse
* fix: a ribbon's dropdown menu (add/sort/regroup/…) could still render clipped near the bottom of the screen – a different overflow source than the one already fixed above (the grid's own scroll container, not the ribbon's collapse trick). It now flips upward when there's no room below, and is capped/scrollable as a last resort
* feat: "collapse all"/"expand all" are now one "collapse ▾" dropdown instead of two separate buttons, and <kbd>Alt+Shift+O</kbd> does the same for the current section as the keyboard equivalent of <kbd>Alt+O</kbd>
* fix: Up/Down/PageUp/PageDown/Left/Right navigation ignored collapsed sections, still stepping through their hidden frames one by one instead of passing straight over them – Left/Right in particular used the plain, section-oblivious frame stepping that ordinary (non-grid) playback also relies on, so it never even considered whether a frame sat inside a collapsed section
* fix: a section holding actual frames was still, under the hood, treated by the keyboard-navigation model as if it were empty – jQuery's own `.find()` gives up on a selector shaped "ancestor descendant" when the "ancestor" part is actually an ancestor of the search root rather than inside it, always coming back empty regardless of what's really there. Every section boundary was quietly forcing the cursor to stop on its own name first before entering its frames
* fix: a row-1 ribbon dropdown (add/sort/collapse) opened right on top of the frame-ops row below it, covering "order ▾"/"regroup ▾" – it now clears that row instead
* fix: <kbd>Alt+O</kbd>/<kbd>Alt+Shift+O</kbd> always collapsed the section the actual current frame lives in, even while the keyboard cursor was pinned elsewhere (an arrow-navigated ribbon) – they now follow the cursor
* enh: the section the keyboard cursor is currently at now colors its own ▶/▼ toggle – whether pinned to a ribbon (arrow-navigated there, or Ctrl+V's destination, which also gets the existing dashed outline) or just being browsed frame by frame, so which section it actually is stays obvious at a glance either way
* fix: after Left/Right moved the cursor to a different column, Up/Down kept snapping back to the column it was in before that move – the remembered "aimed-for" column that keeps vertical navigation in a straight line wasn't forgotten on a horizontal step
