...
## 1.0.0 (unreleased)
* feat (menu): quick play-mode buttons next to Start — **⏱ 5 s** / **⏱ 10 s** launch straight into auto-forward, **🔁 Kiosk** auto-forwards and repeats (loop back at the end); the splash also shows a summary of what's loaded (frames / videos / sections) and a "Recent" list of the last presentations you opened. The now-redundant "Press F1" hint (the command palette replaces it) is gone
* feat: `data-repeat` on `<main>` — at the last frame, wrap back to the first instead of stopping (kiosk / exhibition playback). Toggle live with <kbd>Shift+L</kbd>; presettable from the hash (`#&state=repeat`). Distinct from the existing per-frame `data-loop` (image animation)
