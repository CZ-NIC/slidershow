...
* feat (grid): sticky section headers on all nesting levels – when scrolling through the grid, section headers stay pinned to the top, so you always know which section you're in. Every level stacks and replaces the parent's header as you scroll past it
* feat (grid): outline view (<kbd>Alt+O</kbd>) – toggles a sections-only view of the presentation hierarchy with no thumbnails. Nested sections are expandable, so you can collapse subsections to see just the top-level structure without all the frames
