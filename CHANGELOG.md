...
## 1.0.0 (unreleased)
* feat (tagging): tags are now multi-valued – a frame can carry several tags at once, toggled per digit (<kbd>0</kbd> still clears all); tags can be named via a new "Pojmenovat tagy…" command (`data-tag-names` on `<main>`), and named tags show wherever a tag is displayed (HUD, thumbnails, grid); tagging is now undoable
