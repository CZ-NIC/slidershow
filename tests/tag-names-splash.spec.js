...
test("tag_names()/collect_albums read data-tag-names from <main> even before the first goToFrame (splashscreen)", async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        $(playback.$articles[0]).data("frame").$actor.attr("data-tag", "1")
    })

    expect(await page.evaluate(() => playback.frame.tag_names())).toEqual(["rodice", "vedouci"])

    const albums = await page.evaluate(() => menu.export.collect_albums().map(a => a.name))
    expect(albums).toEqual(["rodice"])
})
