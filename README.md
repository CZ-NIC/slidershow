...
Most viewing options can also be preset from the URL hash so you can hand out a ready-to-run link, e.g. `presentation.html#1&state=duration:5,loop-presentation,progress` starts at frame 1, auto-forwards every 5 s, loops at the end and shows the countdown bar. Available `state=` flags include `duration:<s>`, `loop-presentation`, `progress`, `thumbnails`, `grid`, `properties`, `no-steps`, `editing`, `tagging`.
